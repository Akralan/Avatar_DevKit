...
d'arrière-plan). Il expose 3 endpoints :
  ...
  POST /graft                visage (photo ou .glb) + task_id -> pipeline CPU -> avatar.glb
...
app = Flask(__name__)
# Marge : images du corps + visage (photo brute, ou .glb embarquant son JPEG).
app.config["MAX_CONTENT_LENGTH"] = (MAX_UPLOAD_MB * MAX_IMAGES + 40) * 1024 * 1024
app.wsgi_app = ProxyFix(app.wsgi_app, x_for=1, x_proto=1)

# 1 greffe à la fois (1 worker gunicorn × N threads : 2 greffes simultanées
# se partagent le CPU et peuvent dépasser le timeout 900 s).
# La 2e requête reçoit un 429 immédiat plutôt que de bloquer indéfiniment.
_graft_sem = threading.BoundedSemaphore(1)
...
def graft():
    ...
    if not _graft_sem.acquire(blocking=False):
        log.info("graft 429: une greffe est déjà en cours")
        return jsonify({"error": "Une greffe est déjà en cours, réessayez dans un moment."}), 429
    ...
    try:
        ...
        if _req_bool("local_face") and LOCAL_FACE_GLB.exists():
            ...
            # Deux formes acceptées, et c'est volontaire. Le navigateur envoie un
            # maillage déjà construit — il a MediaPipe sous la main. Une application
            # mobile envoie la photo brute : MediaPipe n'a pas de binding Flutter, et
            # le seul équivalent Android rend 468 points là où cette topologie en
            # exige 478. La construction se fait donc ici, où le modèle est déjà
            # chargé pour l'autre moitié de la greffe.
            if magic != b"glTF":
                from pipeline.face_builder import NoFaceDetected, build_face_glb
                try:
                    glb = build_face_glb(face_path.read_bytes())
                except NoFaceDetected as exc:
                    log.info("face build: %s", exc)
                    return jsonify({"error": "Aucun visage détecté sur la photo.",
                                    "reason": "NO_FACE"}), 422
                except Exception as exc:  # noqa: BLE001
                    log.warning("face build failed: %s", exc)
                    return jsonify({"error": "La photo du visage n'a pas pu être lue.",
                                    "reason": "BAD_FACE_PHOTO"}), 422
                face_path.write_bytes(glb)
                log.info("face mesh built from photo (%d bytes)", len(glb))
        ...
        st = report.step_timings
        log.info(
            "graft ok | method=%s residual=%s | "
            "load=%dms locate=%dms align=%dms stitch=%dms "
            "tone=%dms grain+feather=%dms fringe=%dms material=%dms export=%dms | total=%dms | "
            "warnings=%s",
            report.method, report.landmark_residual,
            st.get("load_ms", 0), st.get("locate_head_ms", 0),
            st.get("alignment_ms", 0), st.get("stitch_ms", 0),
            st.get("tone_ms", 0), st.get("grain_feather_ms", 0),
            st.get("fringe_gutter_ms", 0), st.get("material_ms", 0),
            st.get("export_ms", 0), st.get("total_ms", 0),
            report.warnings,
        )
