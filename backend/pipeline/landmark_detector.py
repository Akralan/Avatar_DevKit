...
# Singleton FaceLandmarker — créé une seule fois, ~1-3 s de chargement évités
# à chaque appel de _detect_2d (qui tourne jusqu'à 3× par greffe via _refine_head_frame).
_landmarker = None
...
def _get_landmarker():
    """Retourne le FaceLandmarker singleton (création lazy, thread-safe)."""
    global _landmarker
    if _landmarker is None:
        with _landmarker_lock:
            if _landmarker is None:
                import mediapipe as mp_mod  # noqa: F401
