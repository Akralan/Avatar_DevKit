...
def _skin_mask(rgb: np.ndarray) -> np.ndarray:
    ...
    lum_ok = (lum > 60) & (lum < 238)
    mask = (r >= g) & (g >= b) & lum_ok
    # Fallback pour peaux foncées / balance des blancs froide (constraint chromatique
    # trop restrictive -> masque < 5 % des pixels candidats).
    if mask.sum() < 0.05 * lum_ok.sum():
        mask = lum_ok
    return mask
...
def measure_body_grain_all(body: trimesh.Trimesh, hf,
                           default_sigma=(5.5, 3.5),
                           default_wl: float = 0.7) -> tuple:
    """Un seul fastNlMeansDenoisingColored sur le crop peau → ((s_lum, s_chroma), wl).

    Évite de débruiter deux fois le corps (une fois pour la sigma, une fois pour la
    longueur d'onde). Appelé depuis l'orchestrateur dans un thread de fond (overlap avec
    les 30-60 s de géométrie/landmarks). Retourne (default_sigma, default_wl) si OpenCV
    ou la texture corps sont absents.
    """
    mat = getattr(body.visual, "material", None)
    img = getattr(mat, "baseColorTexture", None) if mat is not None else None
    if not _CV2 or img is None:
        return default_sigma, default_wl
    import cv2
    rgb = np.asarray(img.convert("RGB"))
    m = _skin_mask(rgb)
    if m.sum() < 500:
        return default_sigma, default_wl
    # Crop au bbox peau : NlMeans beaucoup plus rapide que sur l'atlas 2K entier.
    ys, xs = np.where(m)
    ...
    bgr = cv2.cvtColor(patch, cv2.COLOR_RGB2BGR)
    den = cv2.cvtColor(
        cv2.fastNlMeansDenoisingColored(bgr, None, 7, 7, 7, 21),
        cv2.COLOR_BGR2RGB)
    resid = patch.astype(np.float32) - den.astype(np.float32)

    # --- Sigma (s_lum, s_chroma) ---
    rs = resid[m_crop]
    if rs.shape[0] < 500:
        sigma = default_sigma
    else:
        lum_axis = np.array([1, 1, 1], np.float32) / np.sqrt(3.0)
        lum_proj = rs @ lum_axis
        chroma = rs - lum_proj[:, None] * lum_axis[None]
        sigma = (float(lum_proj.std() / np.sqrt(3.0)),
                 float(np.linalg.norm(chroma, axis=1).std() / np.sqrt(3.0)))

    # --- Longueur d'onde (autocorrélation 1/e, canal vert) ---
    wl = default_wl
    p = resid[..., 1].copy()
    p -= p.mean()
    F = np.fft.rfft2(p)
    ...
    if ac[0, 0] > 0:
        ...
        if k > 0:
            y0v, y1v = prof[k - 1], prof[k]
            wl_raw = (k - 1) + (y0v - thr) / (y0v - y1v + 1e-9)
            wl = float(np.clip(wl_raw, 0.4, 3.0))
...
def add_matched_grain(face: trimesh.Trimesh, body: trimesh.Trimesh, hf,
                      ...
                      body_grain_all: tuple | None = None,
                      stretch_mode: str = "isotropic",
                      streak_sigma: float | None = None) -> trimesh.Trimesh:
    ...
    stretch_band      (lo, hi) foreshortening factors (1/cos theta) mapped by smoothstep
                      to 0..1 fade; ~2.0 == ~60deg tilt starts, ~3.5 == ~73deg full.
    detail_sigma      lowpass sigma (UPSAMPLED px) splitting kept albedo from replaced
                      detail; derived from the grain cell when None.
    stretch_mode      "isotropic" (legacy) | "directional".
                      En mode directionnel on ne lisse la HF QUE selon l'axe vertical (v)
                      dans les zones étirées : les stries horizontales disparaissent, le
                      vrai détail vertical (déjà intact car la projection frontale ne
                      l'étire pas) est conservé.
    streak_sigma      sigma du filtre 1-D vertical (UPSAMPLED px) en mode directionnel ;
                      dérivé de cell_px si None.
    """
    ...
    if body_grain_all is not None:
        _pre_sigma, _pre_wl = body_grain_all
        if sigma is None:
            sigma = _pre_sigma
        _body_wl = _pre_wl
    else:
    ...
    if cell_px is None:
        try:
            ...
            wl = _body_wl if _body_wl is not None else measure_body_grain_wavelength(body, hf)
            cell_px = (face_cell_px(face, body, hf) * upsample * wl * wavelength_scale)
        except Exception:  # noqa: BLE001
    ...
    if supp is not None:
        ...
        detail = arr - low
        w = supp * gate           # pondération combinée étirement + luminance
        if stretch_mode == "directional":
            # Lissage 1-D vertical uniquement : supprime les stries horizontales
            # (artefact de l'étirement u de la projection frontale) sans toucher
            # au vrai détail vertical (non étire par la projection).
            ss = streak_sigma if streak_sigma is not None else max(2.5, 2.5 * cell_px)
            ks = int(6 * ss) | 1   # taille noyau impaire
            k = cv2.getGaussianKernel(ks, ss).astype(np.float32)
            det_v = cv2.sepFilter2D(detail, -1,
                                    np.array([1.0], np.float32), k)   # flou v seul
            out = low + (1.0 - w) * detail + w * det_v + grain * gate
        else:
            # Legacy isotrope : retire toute la HF dans les zones étirées.
            out = low + (1.0 - w) * detail + grain * gate
    else:
        out = arr + grain * gate
