...
def match_skin_tone(face: trimesh.Trimesh, body: trimesh.Trimesh, hf,
                    std_clip=(0.6, 1.6), strength: float = 1.0,
    ...
    `luma_gain_clip` borne le gain appliqué au canal L (CIE Lab).
      - Borne basse 1.0 : on ne comprime JAMAIS le contraste de luminance.
        Un gain L < 1 écraserait la HF (barbe, ombres du nez, pores) jusqu'à -40 %.
        Le shift de moyenne L (vraie carnation) est conservé.
      - Borne haute 1.4 : évite d'amplifier le bruit quand le selfie est plus plat.
      Ancien comportement : luma_gain_clip=(0.6, 1.6).
    """
    import cv2
    ...
    # Canal L : ne jamais comprimer (gain < 1 écrase barbe, ombres, pores).
